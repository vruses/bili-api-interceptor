...
export default defineConfig(({ mode }) => {
  ...
  const fileExt = isMinify ? '.min.user.js' : '.user.js'
  ...
  return {
    ...
    build: {
      minify: isMinify,
      // Overwrite files instead of deleting them for easier debugging in Violentmonkey
      emptyOutDir: false,
    },
    plugins: [
      monkey({
        ...
        server: { open: false },
        build: {
          fileName: packageJSON.name + fileExt,
          externalGlobals: {
